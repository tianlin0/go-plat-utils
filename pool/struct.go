...
func (s *BaseCreator[V]) New() (v V) {
	return v
}

func (s *BaseCreator[V]) ID() string {
	if s.id == "" {
		s.id = reflect.TypeOf(s.New()).String()
	}
	return s.id
}
...
// ClosePool 关闭连接池
func (p *StructPool[O, V]) ClosePool() error {
	closePool[V](p.build)
	return nil
}

// GetOne 从连接池中获取一个连接，使用完毕后需要调用conn.Close()关闭连接
func (p *StructPool[O, V]) GetOne(callFunc func(v V)) error {
	one := get[V](p.build)
	defer func(v V) {
		err := close[V](p.build, v)
		if err != nil {
			log.Println(err)
		}
	}(one)
